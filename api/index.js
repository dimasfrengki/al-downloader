...
const CREATOR=process.env.CREATOR||'SYZHENSU',ADMIN=process.env.ADMIN_PASSWORD||'frengkiz11';
...
async function load(){if(cfg&&!RU)return cfg;try{if(RU){const j=await(await fetch(RU+'/get/cfg',{headers:{Authorization:'Bearer '+RT}})).json();cfg=JSON.parse(j.result||'{}')}else cfg=JSON.parse(fs.readFileSync(FILE))}catch{cfg={}}
 cfg.up=cfg.up||{};cfg.off=cfg.off||[];cfg.set=cfg.set||{};cfg.custom=cfg.custom||[];return cfg}
...
U('downloader','youtube','url','YouTube info + link video & audio',PIPED+'/streams/{id}\nhttps://pipedapi.adminforge.de/streams/{id}\nhttps://api.piped.private.coffee/streams/{id}',j=>({title:j.title,uploader:j.uploader,durasi:j.duration,thumbnail:j.thumbnailUrl,audio:(j.audioStreams||[]).slice(0,3),video:(j.videoStreams||[]).filter(v=>!v.videoOnly).slice(0,3)}));
...
E.filter(e=>e.cat=='downloader'&&e.key).forEach(e=>e.dl=true);
// ===== GAME =====
...
U('search','pinterest','q','Cari gambar (Pinterest, cadangan Openverse)',SZ+'/api/s/pinterest?query={q}\nhttps://api.openverse.org/v1/images/?q={q}&page_size=20',j=>j.results?j.results.map(x=>({title:x.title,image:x.url,thumbnail:x.thumbnail,source:x.foreign_landing_url})):j);
U('search','wallpaper','q','Cari wallpaper (Wallhaven)','https://wallhaven.cc/api/v1/search?q={q}&purity=100&sorting=relevance',j=>j.data.map(x=>({id:x.id,image:x.path,thumbnail:x.thumbs.large,resolusi:x.resolution,source:x.url})));
U('search','spotify','q','Cari lagu (Spotify, cadangan iTunes)',SZ+'/api/s/spotify?query={q}\nhttps://itunes.apple.com/search?term={q}&limit=15',j=>j.results?j.results.map(x=>({judul:x.trackName,artis:x.artistName,album:x.collectionName,cover:x.artworkUrl100,preview:x.previewUrl,url:x.trackViewUrl})):j);
...
// ===== ENGINE =====
const S=(k,env)=>(cfg&&cfg.set&&cfg.set[k])||process.env[env]||'';
async function cobalt(url,audio){
 const bases=(S('cobalt','COBALT_URL')||'https://api.cobalt.tools').split(/[\s,]+/).filter(Boolean);let last;
 for(const b of bases){try{const h={Accept:'application/json','Content-Type':'application/json'},k=S('cobaltKey','COBALT_KEY');if(k)h.Authorization='Api-Key '+k;
  const x=await fetch(b,{method:'POST',headers:h,body:JSON.stringify({url,downloadMode:audio?'audio':'auto',audioFormat:'mp3'}),signal:AbortSignal.timeout(20000)}),j=await x.json();
  if(j.status=='error'||!j.status)throw new Error(j.error?.code||j.text||'HTTP '+x.status);
  return{provider:'cobalt',tipe:j.status,url:j.url,filename:j.filename,pilihan:j.picker}}catch(z){last=z}}throw last}
async function fsaver(url,name,audio){
 const k=S('fastsaver','FASTSAVER_KEY');if(!k)throw new Error('key belum diset');
 const yt=/^yt|youtube/.test(name),x=yt?await fetch('https://api.fastsaver.io/v1/youtube/download',{method:'POST',headers:{'X-Api-Key':k,'Content-Type':'application/json'},body:JSON.stringify({url,format:audio?'mp3':'720p'}),signal:AbortSignal.timeout(60000)})
  :await fetch('https://api.fastsaver.io/v1/fetch?url='+encodeURIComponent(url),{headers:{'X-Api-Key':k},signal:AbortSignal.timeout(30000)});
 const j=await x.json();if(!j.ok)throw new Error(j.detail||'HTTP '+x.status);return{provider:'fastsaver',...j}}
async function proxy(e,q,c){
 const urls=(c.up[e.key]??e.url??'').split('\n').map(s=>s.trim()).filter(Boolean),audio=/mp3|audio/.test(e.name),P=[],errs=[];
 q.id=yid(q.url||'');try{q.path=new URL(q.url).pathname}catch{q.path=''}
 if(e.dl){if(S('fastsaver','FASTSAVER_KEY'))P.push(['fastsaver',()=>fsaver(q.url,e.name,audio)]);if(S('cobalt','COBALT_URL'))P.push(['cobalt',()=>cobalt(q.url,audio)])}
 for(const u of urls)P.push([(u.match(/\/\/([^/{?]+)/)||[])[1]||u,async()=>{
  const x=await fetch(u.replace(/\{(\w+)\}/g,(_,k)=>k=='path'?q.path:encodeURIComponent(q[k]??'')),{signal:AbortSignal.timeout(12000),headers:{'user-agent':'Mozilla/5.0',...e.hdr}});
  if(!x.ok)throw new Error('HTTP '+x.status);const t=await x.text();let j;try{j=JSON.parse(t)}catch{j=t}
  if(j&&typeof j=='object'&&(j.status===false||j.success===false||j.ok===false||j.code<0||(j.error&&!j.data)))throw new Error(String(j.msg||j.message||j.error).slice(0,80)||'ditolak upstream');
  if(e.pick&&typeof j=='object'){let v;try{v=e.pick(j)}catch{}if(v==null){if(e.dl)throw new Error('respons kosong/tidak sesuai');return j}return v}
  if(j===''||j==null)throw new Error('respons kosong');return j}]);
 if(e.dl&&!S('cobalt','COBALT_URL'))P.push(['cobalt-publik',()=>cobalt(q.url,audio)]);
 if(!P.length)throw new Error('Upstream belum diset. Isi lewat menu Admin.');
 for(const[n,f]of P){try{return await f()}catch(z){errs.push(n+': '+z.message)}}
 throw new Error('Semua provider gagal → '+errs.join(' | ')+(e.dl&&!S('fastsaver','FASTSAVER_KEY')?' | Tip: isi FastSaver API key di halaman admin (YouTube/Instagram/Facebook)':''))}
const hdrs=t=>Object.fromEntries((t||'').split('\n').map(l=>l.split(/:(.+)/).slice(0,2).map(v=>v.trim())).filter(a=>a[0]&&a[1]));
const EA=c=>E.concat((c.custom||[]).map(x=>({cat:x.cat,name:x.name,desc:x.desc||'Endpoint custom',params:String(x.params||'').split(',').map(v=>v.trim()).filter(Boolean),url:x.urls||'',key:'c_'+x.cat+'_'+x.name,custom:1,mode:x.mode||'json',stat:x.stat,hdr:hdrs(x.hdr),dl:x.cat=='downloader',
 pick:x.pick?j=>{const v=x.pick.split('.').reduce((a,k)=>a==null?a:a[k],j);return v===undefined?j:v}:null})));
const auth=(q,s,n)=>q.get('x-admin-key')===ADMIN?n():s.status(401).json({status:false,message:'Password admin salah'});
r.get('/list',async(q,s)=>{const A=EA(await load());s.json({status:true,total:A.length,items:A.map(({cat,name,params,desc})=>({cat,name,params,desc}))})});
r.get('/admin/config',auth,async(q,s)=>{const c=await load();s.json({status:true,items:EA(c).map(e=>({path:e.cat+'/'+e.name,mine:!!e.custom,up:!!e.key,def:e.url||'',custom:e.key?c.up[e.key]??null:null,off:c.off.includes(e.cat+'/'+e.name)}))})});
r.put('/admin/config',auth,async(q,s)=>{const c=await load(),{path,url,off}=q.body,e=EA(c).find(x=>x.cat+'/'+x.name==path);
...
r.get('/admin/test',auth,async(q,s)=>{const c=await load(),e=EA(c).find(x=>x.cat+'/'+x.name==q.query.path);if(!e||!e.key)return s.json({ok:false,message:'Bukan endpoint upstream'});
 const p={};e.params.forEach(x=>{const[n,d]=x.split('=');p[n]=d||SMP[n]||'test'});const TU={instagram:'https://www.instagram.com/p/DRsmm9UjKfH/',twitter:'https://x.com/jack/status/20',facebook:'https://www.facebook.com/watch/?v=1393572814172251',spotify:'https://open.spotify.com/track/3zakx7RAwdkUQlOoQ7SJRt',soundcloud:'https://soundcloud.com/issabella-marchelina/sisa-rasa-mahalini-official-audio',threads:'https://www.threads.net/@cindyyuvia/post/C_Nqx3khgkI/',mediafire:'https://www.mediafire.com/file/941xczxhn27qbby/GBWA_V12.25FF-By.SamMods-.apk/file'};if(p.url)p.url=TU[e.name]||(/^yt|youtube/.test(e.name)?'https://youtu.be/dQw4w9WgXcQ':p.url);
 if(e.mode&&e.mode!='json')return s.json({ok:true,ms:0,preview:'Mode '+e.mode+' (tidak perlu tes upstream)'});const t=Date.now();try{const o=await proxy(e,p,c);s.json({ok:true,ms:Date.now()-t,preview:JSON.stringify(o).slice(0,300)})}catch(z){s.json({ok:false,ms:Date.now()-t,message:z.message})}});
r.get('/admin/settings',auth,async(q,s)=>{await load();const m=v=>v?'••••'+v.slice(-4):'';s.json({status:true,fastsaver:m(S('fastsaver','FASTSAVER_KEY')),cobalt:S('cobalt','COBALT_URL'),cobaltKey:m(S('cobaltKey','COBALT_KEY'))})});
r.put('/admin/settings',auth,async(q,s)=>{const c=await load();for(const k of['fastsaver','cobalt','cobaltKey']){const v=q.body[k];if(typeof v=='string'&&v){if(v=='-')delete c.set[k];else c.set[k]=v.trim()}}await save();s.json({status:true})});
r.put('/admin/replace',auth,async(q,s)=>{const{from,to}=q.body;if(!from||!to)return s.status(400).json({status:false,message:'from & to wajib'});const c=await load();let n=0;
 for(const e of EA(c).filter(x=>x.key)){const cur=c.up[e.key]??e.url??'';if(cur.includes(from)){c.up[e.key]=cur.split(from).join(to);n++}}await save();s.json({status:true,diubah:n})});
r.get('/admin/custom',auth,async(q,s)=>{const c=await load();s.json({status:true,items:c.custom})});
r.put('/admin/custom',auth,async(q,s)=>{const c=await load(),b=q.body||{},ok=/^[a-z0-9_-]{1,30}$/,t=(v,n)=>String(v||'').slice(0,n);
 b.cat=t(b.cat,30).toLowerCase().trim();b.name=t(b.name,30).toLowerCase().trim();
 if(!ok.test(b.cat)||!ok.test(b.name)||['admin','list'].includes(b.cat))return s.status(400).json({status:false,message:'Kategori & nama: huruf kecil, angka, - atau _ (maks 30)'});
 if(E.some(e=>e.cat==b.cat&&e.name==b.name))return s.status(409).json({status:false,message:'Nama itu sudah dipakai endpoint bawaan'});
 if(b.mode!='static'&&!t(b.urls,1).trim()&&b.cat!='downloader')return s.status(400).json({status:false,message:'URL upstream wajib diisi'});
 const it={cat:b.cat,name:b.name,desc:t(b.desc,120),params:t(b.params,200),urls:t(b.urls,2000),pick:t(b.pick,100),mode:['json','redirect','static'].includes(b.mode)?b.mode:'json',stat:t(b.stat,5000),hdr:t(b.hdr,500)};
 c.custom=c.custom.filter(x=>!(x.cat==it.cat&&x.name==it.name)).concat(it);await save();s.json({status:true})});
r.delete('/admin/custom/:cat/:name',auth,async(q,s)=>{const c=await load();c.custom=c.custom.filter(x=>!(x.cat==q.params.cat&&x.name==q.params.name));delete c.up['c_'+q.params.cat+'_'+q.params.name];await save();s.json({status:true})});
r.get('/admin/export',auth,async(q,s)=>s.json(await load()));
r.put('/admin/import',auth,async(q,s)=>{const b=q.body,c=await load();if(!b||typeof b!='object'||Array.isArray(b))return s.status(400).json({status:false,message:'Format salah'});
 c.up=b.up&&typeof b.up=='object'?b.up:{};c.off=Array.isArray(b.off)?b.off:[];c.custom=Array.isArray(b.custom)?b.custom:[];if(b.set&&typeof b.set=='object')c.set=b.set;await save();s.json({status:true})});
r.all('/:cat/:name',async(req,res)=>{
 const c=await load(),e=EA(c).find(x=>x.cat==req.params.cat&&x.name==req.params.name);
 if(!e)return res.status(404).json({status:false,message:'Endpoint tidak ada. Lihat /api/list'});
 if(c.off.includes(e.cat+'/'+e.name))return res.status(503).json({status:false,message:'Endpoint dinonaktifkan admin'});
 ...
 try{const fill=(t,enc)=>String(t).replace(/\{(\w+)\}/g,(_,k)=>enc?encodeURIComponent(q[k]??''):q[k]??''),o=e.fn?await e.fn(q,req):e.mode=='static'?(()=>{const t=fill(e.stat);try{return JSON.parse(t)}catch{return t}})():e.mode=='redirect'?{redirect:fill((c.up[e.key]??e.url).split('\n')[0],1)}:await proxy(e,q,c);
